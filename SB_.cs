...
, () => { WaitSec_(5000); }
    ...
    public static class Check
    {
        ...
        {
            //SocketClient sr = new SocketClient();
            //sr.Send();

            var a = Prime.GO(new int[] {12,15 } );
            ...
            if (start_ <= input_.Length)
            {
                if (EncoutedType(input_[start_]) == typeof(Number))
                {
                    ...
                    for (int i = start_; i < input_.Length; i++)
                    {
                        ...
                        for (int i2 = i; i2 < input_.Length-1; i2++)
                        {
                            ch_ = input_[i2];
    ...
    {
        static int _res;
        static double _thres;
        static double _content;
        static double _evap_per_day;

        public static int evaporation(double content, double evap_per_day, double threshold)
        {
            _res = 0;
            _thres = 0;
            _content = 0;
            ...
            _content = content;
            _thres = (_content * (threshold / 100));
            _evap_per_day = evap_per_day;
            evap();
            return _res;
        }

        public static void evap()
        {
            _res += 1;
            _content = _content - (_content * (_evap_per_day / 100));
            while (_content > _thres)
            {
                evap();
            }
        }
    }
    public class Evaporator2
    {
    ...
    public static class MaskifyKata
    {
        public static string Maskify(string cc)
        {
            ...
            //if (len < 4) { throw new Exception(@"Less than four. Length " + len); }
            if (len > 4)
            {                
                for (int i = 0; i < len; i++)
                {
            ...
            return string.Join(null, ch);
        }
    }
    
    public static class GenerateRangeKata
    {
        public static int[] Generate(int min,int max, int step)
        {
            int[] arr;
            
            if (min > max) { int a = max; min = max; max = a; }
            
            if (step > 0) { 
                double dbl = (max - min) / step;
                double MathRnd = Math.Round(dbl, 0, MidpointRounding.AwayFromZero);
                arr = new int[(int)MathRnd + 1];
    ...
    public static class GNAkata
    {
        public static string GNA(string input)
        {
            var a = from s in input where (s == 'A' ||  s == 'T' ||  s == 'C' ||  s == 'G') select s;
            char[] arr = a.ToArray();
            char[] arr2 = new char[arr.Length];
            for (int i= 0; i< arr.Length; i++)
            {
                if (arr[i] == 'A') { arr2[i] = 'T'; } else {
                    if (arr[i] == 'T') { arr2[i] = 'A'; }
                }
                if (arr[i] == 'C') { arr2[i] = 'G'; }
                else
            ...
            return string.Join(null, arr2);
        }
    }
    
    public static class IntSumKata
    {
        public static int Pos(int[] arr)
        {
            ...
            for(int i = 0;i<arr.Length;i++)
            {
                ...
                if (countArrLeft(arr, i) == countArrRight(arr, i + cnt))
                {
                    return i;
                }
            }
        ...
        public static int countArrRight(int[] arr,int num)
        {
            int sum = 0;
        ...
        public static int countArrLeft(int[] arr, int num)
        {
            int sum = 0;
            for (int i = 0; i < num; i++)
            {
    ...
    public static class NupcountKata
    {
        public static int GO(int[] arr,int n)
        {
            int result = 0;
            int sum = 0;
            bool upped = false;
    ...
    public static class DoublesKata
    {
        public static int GO (string input)
        {
            List<char> chDoubled = new List<char>();
            ...
            for(int i =0;i< len-1; i++)
            {
                ...
                {
                    if ((arr[i] == arr[i2] || Char.ToUpper(arr[i]) == Char.ToUpper(arr[i2])) &&
                        !(from s in chDoubled where Char.ToUpper(s) == Char.ToUpper(arr[i]) select s).Any()
                        ) { chDoubled.Add(arr[i]); }
                }
            }
            result = chDoubled.Count();
            return result;
        }
    }

    public static class ExpKata
    {
        ...
        public static long EXP(long a)
        {
            ...
            {
                exp += 1;
                double expRes=Math.Pow(Char.GetNumericValue(ch), exp);
                result += Convert.ToInt32(expRes);
            }
            return result;
        }
    }
    ...
    public static class Prime
    {
        ...
        public static int[] FactorsArr(int[] arr)
        {            
                foreach(int i in arr)
                {
                    ...
                    {
                        if(i % i2 == 0) {                      
                            if(PrimeDetect(i2) && !(from s in primes where s == i2 select s).Any())
                            {                            
                                primes.Add(i2);
                            }
                        }
        ...
        public static bool PrimeDetect(int a)
        {
            bool result = true;
            if (a > 1)
            {
