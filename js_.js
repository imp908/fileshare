...
function bindEvent(obj_,event_,func_){
  if(!isNull(obj_)){
    if(!isNull(event_)){
      if(!isNull(func_)){
        obj_.addEventListener(event_,func_,true);
      }
    }
  }
...
function addCSS(obj,className_){
  if(!isNull(obj)){
    if(!isNull(className_)){
      obj.classList.add(className_);
    }
  }
}
function removeCSS(obj,className_){
  if(!isNull(obj)){
    if(!isNull(className_)){
      obj.classList.remove(className_);
    }
  }
}
function toggleCSS(obj,className_,true_){
  if(!isNull(obj)){
    if(!isNull(className_)){
      obj.classList.toggle(className_,true_);
    }
  }
}
function containsCSS(obj,className_){
  if(!isNull(obj)){
    if(!isNull(className_)){
      return obj.classList.contains(className_);
    }
  }
}
...
function XHR( method_, url_, body_,async_,cross_, user_, pass_, ){
  console.log("XHR");
  var xhr_= new XMLHttpRequest();
  xhr_.onreadystatechange=XHR_statechange();

  xhr_.addEventListener("load", XHR_load);
  xhr_.open(method_,url_,async_,user_,pass_)

  if(cross_==true){
    xhr_.withCredentials=true;
  }
...
function XHR_statechange(){
  console.log(["XHR_statechange",this]);
}

function XHR_load(){
  console.log(["XHR_load",this,this.response,this.responseText]);
}
function XHR_onload(){
  console.log(["XHR_onload",this,this.response,this.responseText]);
}
...
//Binds to class selected CSS class
...
function XHRtest(){
  console.log("XHRtest")
  XHR("GET","http://msk1-vm-inapp01:8186/api/News2/2","",true,true,"","");
}
...
function addButtonWithEvent(){
  clearBody();

  //create button

  var btn=createElement(curDoc_(),"button");
  btn=appendChild_(curBody_(),btn);
  btn.style.zIndex=10000;
  btn.innerHTML="XHRtest"
  btn.style.marginTop="10vh";
  btn.style.marginLeft="5vw";
  addCSS(btn,"btn")
  addCSS(btn,"btn-primary")
  bindEvent(btn,"click",XHRtest);

  var tb=createElement(curDoc_(),"input");
  tb=appendChild_(curBody_(),tb);
  tb.style.zIndex=10000;
  tb.style.marginTop="10vh";
  tb.style.marginLeft="6vw";
  setAttribute(tb,"type","text");
  console.log(btn);
