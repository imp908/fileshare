...
https://angular.io/guide/setup-systemjs-anatomy
		...
		public void CMD_commands(){
			...
			//update local npm when global Angular CLi is greater then local
			//while ng serve --open
		...
		public void CLI_boilerplates{
			cd .../root_folder
			ng new AppName
			cd AppName/App
			ng g module AppNew
			cd AppNew
			//create compoentn for new app with html,css e.t.c
			ng g component AppNew -flat
...
componentModuleShema[
	...
	inject
		...
		//services and classes can be 
		//Imported -> new instance
		//or Injected -> singleinstance (static)
		InjectService[
		...
		//every exported class can be injected as service or @Injecatbale() class
		//?? @Injecatbale() -> directive is for good practice only??
		InjectClass{
			
			ModelFile.ts{
				export class ToInject{...}
			}
			App.ts{
				import {ToInject} from '.../ModelFile'
				...
				@NgModule({
					...
					,Providers:[ToInject]
			...
			compotent.ts{
				constructor(private clImpInj_: ToInject){
