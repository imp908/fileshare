...
{
	//Usefull links
	public protected class mostUsefullLinks()
	{
	//dev guide
	...
	#region C#
	//Forums
	public protected class stackoverflow
	{
	...
	*/
	...
	//structure
	public protected class OverallC#
	{
		public  Properties(){
	...
	public protected class External_Libraries(){
	//reading excel
	http://epplus.codeplex.com/
	}

	//app setting connstrings example
	public protected class ConnectionStrings(){
	
		
		//connection strings
		public internal connectionStrings()
		{
		...
		public internal fromAppSettings()
		{
	...
	public protected class IssuesErrors(){
	...
	rowcount = oracleTableCount.ExecuteScalar().ToString();
		...
		public internal class serviсe_queries()
		{
	...
	createTable = "create table MERCHANT_LIST (ID_MERCHANT varchar(50))";
		...
		public internal class Serealization()
		{
			
			public internal BinaryFormatter()
			{
							
				public Serialize(String path, List<OracleQueriesListClass> myArray)
				{
		...
		public internal class ExcelIntegration()
		{
	...
	workbookPart.Workbook.Save();
		...
		public internal class EntityFramework()
		{
			public TrullyGenericRepo()
			{
				
				public internal CallStoredProcedure()
				{
					/*
					1) create stored procedure
					2) create fake empty POCO FakePOCO
					3) bind SqlQuery to kafe POCO an call with procedure name
					*/
		...
		//mvc tempaltes
		public internal class  MVC_templates()
		{
	...
	* */
	...
	//copy pasy excel range to powwerpoint
	...
	*/
	...
	select 
	card,-- номер карты
	trn_DATE,-- дата операции
	local_amnt as RUR,-- сумма в рублях
	merchant, abrv_name, mcc,
	case when substr(POINT_CODE,1,1)='2' then 'нечиповый' 
		 when substr(POINT_CODE,1,1)='5' then 'чиповый' 
		  else substr(POINT_CODE,1,1) end as Device,
	case when substr(POINT_CODE,7,1)='2' then 'мп частично' 
		 when substr(POINT_CODE,7,1)='5' then 'чип' 
		 when substr(POINT_CODE,7,1)='6' then 'ручной ввод'
		 when substr(POINT_CODE,7,1)='8' then 'fallback/мп'
		 when substr(POINT_CODE,7,1)='9' then 'чтение мп'
		 when substr(POINT_CODE,7,1) in ('S', 'T', 'U', 'V') then 'e-comm'
		  else substr(POINT_CODE,7,1) end as Input_mode,
	case when substr(POINT_CODE,8,1)='1' then 'ПИН'
		 when substr(POINT_CODE,8,1)='5' then 'Подпись'
