	...
	*.config
	...
	git rm -r NewsAPI.Tests/Properties
				...
				[
					...
					ADD UNPUSHED LAST CHANGES TO LAST COMMIT
					git add .
					git commit --amend
		...
		public Issuer Trackers
		{
			...
			Jira[
				...
				//find projects i worked on
				assignee was i.neprintsev
				//all assigned on last H
				assignee changed to currentUser() after -84h
			]
		...
		public Examples()
		{
			...
			{			
				...
				structs vs classes[
					...
					- strored in stack compared by value not reference
...
is check type 
as safe check ? assign : null with overriding/new functionality
				...
				events[
					...
					-EventArgs created as class to be passed by ref to all subscribbers					
					- no need to inherit EventArgs in Core
					
					boilerPlate[
						//Emitter
						//event arg
						Public class argsName:EventArgs {};
						
						public class EventEmitter {...
							//event type
							public event EventHandler<argsName> eventName;					
							//event firing method
							public virtual void eventHandlerMethod(argsName e){
								EventHandler<argsName> handler=eventhandler
						...
						public void eventReceiver{...
							public void receiver(object o,argsName e){...}
						...}
						EventEmitter.eventName+=eventReceiver.receiver;
					]
					
					//invoke event with testing for emty subscribers
					eventName?.Invoke(this, new argsName(){...} );
					//empty args
					EventArgs.Empty;					
				...
				async[
					...
					TPL[
						Task->
							async futures 
							by default run on the same thread
							Task,Run explicit multithrading
					]
...
{"statement" : "create (v_name1:v_typeb{prop1:'v_name1', propn:'v_prop1n'})-[n1:e_type{e_name:'v_name1' + '-->' + 'v_name2'}]->(v_name2:v_typea{prop1:'v_name2', propn:'v_prop2n'})<-[n2:e_type{e_name:'v_name3' + '-->' + 'v_name2'}]-(v_name3:v_typeb{prop1:'v_name3', propn:'v_prop3n'})"},{"statement" : "create (v_name4:v_typeb{prop1:'v_name4', propn:'v_prop4n'})-[n3:e_type{e_name:'v_name4' + '-->' + 'v_name5'}]->(v_name5:v_typea{prop1:'v_name5', propn:'v_prop5n'})<-[n4:e_type{e_name:'v_name6' + '-->' + 'v_name5'}]-(v_name6:v_typeb{prop1:'v_name6', propn:'v_prop6n'})"},{"statement" : "match(a:v_typea{prop1:'v_name2'}),(b:v_typea{prop1:'v_name5'}) merge (a)-[:e_type2{name:'a->b'}]->(b)"},{"statement" : "match(a:v_typea{prop1:'v_name2'}),(b:v_typea{prop1:'v_name5'}) merge (b)-[:e_type3{name:'b->a'}]->(a)"},{"statement" : "match(a:v_typea{prop1:'v_name2'}),(b:v_typea{prop1:'v_name5'}) merge (b)-[:e_type{name:'b->a'}]->(a)"},{"statement" : "match(b:v_typeb{prop1:'v_name4'}),(a:v_typeb{prop1:'v_name3'}) merge (a)-[:NEW_REL]->(b)"}
		...
		public void MongoDb()
		{
			...
			queries[
				...
				db.snapshots.find({PrevId: {$type: 1}})
				
				//Add property join from another collection
				db.cards.find({ Title: { $exists: false } }).forEach(function (doc1) {
					var doc2 = db.snapshots.findOne({ "LayoutId": doc1._id });
					if (doc2 != null) {
						db.cards.update({ "_id": doc1._id }, { $set: { "Title": "" } }, false, true);
						db.cards.update({ "_id": doc1._id }, { $set: { "Title": doc2.Properties.Title } }, false, true);
					}
				});

				//remoove property
				db.cards.find({ Title: { $exists: true } }).forEach(function (doc1) {
					db.cards.update({ "_id": doc1._id }, { $unset: { Title: "" } });
				});
		...
		public Windows
		{
			public CMD
			{
				...
				public void Unsorted()
				{
					...
					dxcpl -> start directx command panel
					wmic csproduct get name -> get laptop model
					netsh winsock reset -> reset net
				}
