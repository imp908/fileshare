...
namespace Repo
{
    ...
    class REPO
    {
        ...
        public static void InstantiateFromType()
        {
            ...
            TypeTpMethodContainer tc = new TypeTpMethodContainer();
            string name_ = @"KEY_CLIENTS";
            var types_ = AppDomain.CurrentDomain.GetAssemblies().SelectMany(s => s.GetTypes()).Where(s => s.Namespace == @"Repo.DAL");
            var type_ = (from s in types_ where s.Name == name_ select s).FirstOrDefault();
            var inst = Activator.CreateInstance(type_);
            var properties = inst.GetType().GetProperties();
            PropertyInfo propertyInf = null;

            tc.PropertiInfoValueSet(properties, propertyInf, "SE_NUMBER", inst, 9290000010);
            tc.PropertiInfoValueSet(properties, propertyInf, "MERCHANT", inst, 9290000010);
            tc.PropertiInfoValueSet(properties, propertyInf, "SECTOR_ID", inst, 9);

            tc.MethodGet(name_);
            tc.MethodInit((KEY_CLIENTS)inst);
            tc.Save();
        }
        ...
        public delegate void AddMethod(IMerchant ent_);
        public class TypeToMethod
        {
            public TypeToMethod(Type entity_type_, AddMethod method_,string type_name_)
            {
        ...
        public class TypeTpMethodContainer
        {
            public List<TypeToMethod> typesMethods;
            ...
            public TypeTpMethodContainer()
            {
                ent = new SQL_entity();
                uof = new Edit_UOF();

                Type t_ = typeof(KEY_CLIENTS);
                AddMethod am_ = uof.AddKK;
                List<TypeToMethod> typesMethods = new List<TypeToMethod>();

                this.typesMethods = new List<TypeToMethod>();
                this.typesMethods.Add(new TypeToMethod(t_, am_,@"KEY_CLIENTS"));
                this.typesMethods.Add(new TypeToMethod(typeof(REFMERCHANTS), uof.AddREF, @"REF_MERCHANTS"));
            }
            public void MethodGet(Type type_)
            {
                this.am = (from s in this.typesMethods where s.entity_type == type_ select s.method).FirstOrDefault();               
            }
            public void MethodGet(string name_)
            {
                this.am = (from s in this.typesMethods where s.type_name == name_ select s.method).FirstOrDefault();
            }
            ...
            public void PropertiInfoValueSet(PropertyInfo[] piArr_,PropertyInfo pi_, string name_, object obj_,object val_)
            {
    ...
    public class Edit_UOF
    {
        ...
        SectorFilterRepo<KEY_CLIENTS> KeyKLients;

        public Edit_UOF()
        {
            ent = new SQL_entity();
            ACQ_D = new EditRepo<T_ACQ_D> (ent);            
            REFMERCHANTS = new UserFilterRepo<REFMERCHANTS>(ent);
            KeyKLients = new SectorFilterRepo<KEY_CLIENTS>(ent);
        }
        ...
        {
            KeyKLients.AddEntity((KEY_CLIENTS)entity_);
        }
        public void DeleteBySectorID(int id_)
        {
            KeyKLients.DeleteBySector(id_);
        }
