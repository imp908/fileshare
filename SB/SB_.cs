...
, () => { WaitSec_(5000); }
    ...
    public static class ListLast
    {    
        public static void GO()
        {
            List<User_> us = new List<User_>() {
                new User_ { ID = 1, Name = @"Name1" },
                new User_ { ID = 2, Name = @"Name2" }
            };

            var a = us.Last();
            us.Add(new User_ { ID = 1, Name = @"Name3" });
            a = us.Last();
            var ss = us.Select(s => s.Name);
            a = us.Last();
        }
    }

    //compare by ref and val + tuplets
    public static class Compare
    {
        public static void GO()
        {
            Tuple<int, int, int> tp1 = new Tuple<int, int, int>(1, 1, 1);
            Tuple<int, int, int> tp2 = new Tuple<int, int, int>(1, 1, 1);
            string a =
                (tp1.Equals(tp2) ? "{=}" : "{!=}") + " ; " +
                (tp1.Item1 == tp2.Item2 ? "{=}" : "{!=}") + " ; " +
                (tp1 == tp2 ? "{=}" : "{!=}");

            List<int> ls1 = new List<int>() { 1, 1, 1 };
            List<int> ls2 = new List<int>() { 1, 1, 1 };
    ...
    //protected internal check
    public class PrivateCheck
    {
        protected int var;
    }
    public class PC2 : PrivateCheck
    {
        public void add()
        {
            base.var += 1;
