...
rezume[
	...
	Стеки на которых есть код:
		MVC WebApi,WebApi Core 2.0,Angular CLI,jQuery,D3
	
	Проекты кратко:
		трехзвенки, EntityFramework , Generic repository + UOW, MVC: xhtml Razor + ajax CRUD.
		Micro ORM expression tree C# -> NoSQL. 
	...
	Проекты подробно: [
		...
		ранее [
			1 год 
			Back:[
				админки аналитических сайтов к базам Oracle и MSsql позже тот же код к EF Framework пере использовался для Postgre.
				Сайт на MVC + Razor forms + JS ajax CRUD + фильтрация контента на страницах.
				100 таблиц на Oracle из них аггрегация 50 таблиц MSsql далее на них порядка 10 отчётов.
				Загрузка/выгрузка в Excell, миграция из внутреннего Oracle в MSsql (из-за больших объемов sql через райвер)]
			Front:[
				MVC 5 forms и JS фильтры по бизнесспараметрам,авоматизация отчётов для аналитиков SQL->Excel plain table
				(могли писать срипт SQL, выбирать периодичность и список почты рассылки), импорт экспорт excel таблиц, графики ]
				на D3.js (привязка к карте России с geo.json).
			]
		...
		в НСПК:[
			...
			1 год
				Back:[
					C#, NoSQL
					Micro ORM к NoSQL через HTTP, аутентификация, анонимный вход, отсылка POST,GET,PUT,DELETE, чтение ответов, NewtonJSON generic сериализация/десериализация. 
					3 месяца первый генератор базы из кода. 6 мес полный CRUD + 5 UOW с бизнес логикой. далее рефакторинг под изменение БТ. 1 мес реализация Expression trees.
					(7К рабочего кода, ~150 тестов.) Вывод через WebApi позже WebApi Core. Generic Repository переводит запросы с классами в синтаксис NoSQL.
					Част переписана, часть дописывается на expression trees, получается как Method-Based Query Syntax. CRUD, генерация базы CODE first, миграция с сервера на сервер, 
					получение/выгрузка метаданных базы,Units Of work с бизнесс логикой. Есть тесты для CRUD Neo4j можно ипользовать для обращения к драйверу MongoDB. Переписывается на Core 2.0.
					Тестирование API c разными вводами + ожидаемый ответ, NTLM, анонимный, вывод результатов в JSON.
					+ часть архитектуры для NoSQl - как отображать комментарии, лайки таги, должности пользователей, что оправить на связи что на ноды.]
				Front:[
					TS,Angular 5(6)
					1 мес концепция без генератора , 1 мес новая модель генерация контроллов, 0,5 мес новая модель и генерация и отрисовка и контролов и кнопок и самих объектов.
					Доработка бизнес портала. Отдельные тестовые блоки для ajax angular js, правка ошибок, доработка RichText с использование CKeditor.
					Написание функционального прототипа опросов , конструктор HTML на Angular 5 с генерацией часто повторяющихся элементов 
					из модели TypeScript (кнопки New,Delete,update,save, Объекты Quiz,Question,Answer)
					Возможность из кода, коллекциями классов, обрисовывать формы с большим количеством элементов и быстро менять порядок, набор элементов и задавать css класс на коллекцию, 
					например row col bootstrap или flex-container row,column.
					есть -> checlbox, radiobutton, dropbox, text input, datepicker, number picker, label. 
					Http Post Get в NoSQL.
					обращение к бэку через WebApi Core 2.0]
				]	
	]
	
	Процесс:
		Сначала проверка концепции несколько часов дней, затем архитектура дни, затем модель дни, затем бизнесс логика на бек  если БЛ не понятна самому заказчику могут уйти недели,
		на фронте event схема. Далее генераторы мок даных для тестов, затем тесты. Затем из тестов сборка самой основы.
		Тестируется функционально всё, интеграционно многое, бек и фронт пишутся параллельно на моканых генерируемых данных, потом сводятся и тестятся API тестером (GET,POST,PATCH)
		и из под Fiddlera. 
		Тестируем локально, бек и фронт, затем  деплой. Деплой webapi из vs авто. 
	
	ПО:
		Git (Hub,Lab,Stash), Mercural.
		Notepad++, VsiaulStudio (13-17), VS code, Atom.
		Swageger.
		Jira,Confluence.
	
	В процессе работы вне кода:
		Доработка/корректировака/консультирование коллег(руководство, смежные подразделения) по БТ/ФТ/ТЗ.
		Обяснение концепциий внутренним заказчикам для общения с вендорвами (POCO, модель данных, прототипы, обобщенный код, event схема, роли, стеки)
		Участие в процедуре выбора подрядчиков - проработка сценария взаимодействия, списка и параметров выбора стека из сочетаний
		(NoSQL,SQL + Core,C#,NodeJS + React,Angular(IO,JS)).
		Предложена модель графовой базы - полностью перешли на рекоммендуюмйю так как показала и гибкость и скорость.		
		Рефакторинг нескольких разрозненных проектов в библиотеку с разделением на функциональные блоки Http, Json, Token builder, Repository, UOW, manager.
		branch merge,merge conflict solve, отработка и написание manual по: деплой из VS, ветвение в git, перенос из tfs в git, 
		описание нового кода архитектура и как пользоваться. Выявление причин и рекоммендации по исправлению багов коллегам.
