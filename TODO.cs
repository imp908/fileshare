...
которые служба, посмотрев на почтовый ящик, перенесла в БД.
				...
				preliminary[
					...
					//sending turn on
					Inter02: C:\Program Files (x86)\Intravision\IntraService Agent 4.43.3.0603
					//врубить сервис в iis service
					C:\Program Files (x86)\Intravision\IntraService Agent 4.45.3.0603				
					//mail in [Smtp]
					//real send mail in config help@nspk.ru
					base from address help@nspk.ru Qwer1234 
					//[is4_32_5].[dbo].ImportMailAccount
					//from field override
					//subject to parse trigger network_incident_importmail @Mail_from @subject
					//change ProvidersSync net@nspk.ru
	...
	dataflow
	log
	select * from is4_32_5.[dbo].[ImportMailMessageFile]
	log 
	select * from is4_32_5.dbo.ImportMails
	with dll
	-> select * from is4_32_5.dbo.task data
	select * from is4_32_5.dbo.TaskExecutor
