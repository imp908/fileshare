...
namespace NSQLManagerTests.Tests
{
    ...
    public class CommandTest
    {
        ...
        public CommandTest()
        {
            ...
            this.commandShemas = new CommandShemas(new CommandBuilder(), new FormatFromListGenerator(new TextToken()), new TextToken());
            this.commands = new Commands(new OrientCommandBuilder(), new FormatFromListGenerator(new TextToken()), new TextToken());
        }
        ...
        public void UniversalCommandCheck()
        {
            ...
            Assert.Equal("Select Name , GUID from", select);
        }
        ...
        public void NestedCheck()
        {
            ...
            this.commands.Select().Nest(new OrientRoundBraketLeftToken(),new OrientRoundBraketRightToken(), new TextToken() {Text= string.Empty });

            string result = this.commands.GetCommand();
        ...
        [Fact]
        public void SelectFromCheck()
        {
            string result = this.commandShemas.Select().GetText();
            Assert.Equal("Select from", result);
        }
        [Fact]
        public void SelectFromParametersCheck()
        {            
            commandBuilder = new CommandBuilder(new List<ITypeToken> { new TextToken() { Text = "Name,GUID" } }, new TextToken() { Text = "{0}" });
            string result = this.commandShemas.Select(commandBuilder).GetText();
            Assert.Equal("Select Name,GUID from", result);
        }
