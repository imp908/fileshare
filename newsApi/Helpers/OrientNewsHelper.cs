...
namespace NewsAPI.Helpers
{
    public class OrientNewsHelper
    {
        ...
        public Dictionary<string, string> PrepareBirthdaysDataToSend(IHttpActionResult ar)
        {
            Task<HttpResponseMessage> returnedTask = ar.ExecuteAsync(new CancellationToken());

            using (var contentStream = returnedTask.Result.Content.ReadAsStreamAsync().Result)
            {
                using (var reader = new StreamReader(contentStream, Encoding.UTF8))
                {
                    // после получения ответа потребуется посчитать количество возвращенных записей
                    ...
                    var recipientsWithContent = from t in targetNoResultPart
                                                group t["bdayman"] by t["mailto"] into g
                                                select new { recipient = g.Key, content = g.ToList() };

                    var birthdays = recipientsWithContent.ToList();
                    ...
                    if (birthdays.Count > 0)
                    {
                        ...
                        {
                            string messageBody = "Добрый день! Сегодня день рождения у следующих ваших друзей в избранном: ";

                            for (int j = 0; j < birthdays[i].content.Count; j++)
                            {
                                messageBody += birthdays[i].content[j];
                                if (j != birthdays[i].content.Count - 1)
                                {
                                    messageBody += " , ";
                                }
                                else { messageBody += "! "; }
                            ...
                            messagesToSend.Add(birthdays[i].recipient.ToString(), messageBody);
                        }
                    }
        ...
        public class ReturnPersonRelationCount : IHttpActionResult
        {
            Task<HttpResponseMessage> returnedTask;

            public ReturnPersonRelationCount(IHttpActionResult ar)
            {
                this.returnedTask = ar.ExecuteAsync(new CancellationToken());
            }
            ...
            async public Task<HttpResponseMessage> ExecuteAsync(CancellationToken cancellationToken)
            {
                ...
                string currentPersonRelations_actual;
                using (var contentStream = returnedTask.Result.Content.ReadAsStreamAsync().Result)
                {
                    using (var reader = new StreamReader(contentStream, Encoding.UTF8))
                    {
                        ...
                        var prcToken = targetExactlyFull.FirstOrDefault();

                        JToken prc;
                        bool prcExist = JObject.Parse(prcToken.Value<string>()).TryGetValue("personRelationCount", out prc);
                        if (prcExist)
                        {
                            currentPersonRelations_actual = prc.ToString();
                        }
                        else { currentPersonRelations_actual = "no values"; }
                    }
                }

                var resp = new HttpResponseMessage(HttpStatusCode.OK);
                resp.Content = new StringContent(currentPersonRelations_actual, Encoding.UTF8, "text/plain");

                return await Task.FromResult(resp);
            }
        }

        public class ReturnPersonRelations : IHttpActionResult
        {
            Task<HttpResponseMessage> returnedTask;

            public ReturnPersonRelations(IHttpActionResult ar)
            {
                this.returnedTask = ar.ExecuteAsync(new CancellationToken());
            }
            ...
            async public Task<HttpResponseMessage> ExecuteAsync(CancellationToken cancellationToken)
            {
                ...
                using (var contentStream = returnedTask.Result.Content.ReadAsStreamAsync().Result)
                {
                    using (var reader = new StreamReader(contentStream, Encoding.UTF8))
                    {
                        ...
                        var targetwithResultPart = JObject.Parse(requestedEntity_json);
                        var targetNoResultPart = targetwithResultPart.SelectToken(String.Format("result"));
                        var personRelations = targetNoResultPart.Select(s => s["personRelations"][0]);
                        json = JsonConvert.SerializeObject(personRelations);
                    }
        ...
        public class SendPersonRelationBirthdaysNotifications
        {
            Dictionary<string, string> messagesToSend;

            public SendPersonRelationBirthdaysNotifications(Dictionary<string, string> messagesToSend)
            {
             ...
             public void Send()
            {
                ...
                {
                    new ExchangeNotificationSendHelper().SendNotificationExchange("Дни рождения ваших друзей!",item.Key,item.Value);           
                }
            ...
            async public Task<HttpResponseMessage> ExecuteAsync(CancellationToken cancellationToken)
            {
                ...
                using (var contentStream = returnedTask.Result.Content.ReadAsStreamAsync().Result)
                {
                    using (var reader = new StreamReader(contentStream, Encoding.UTF8))
                    {
                        ...
                        var targetNoResultPart = targetwithResultPart.SelectToken(String.Format("result"));
                        //     var targetExactlyFull = targetNoResultPart.Select(s => s["this"]);
                        var targetExactlyFull = targetNoResultPart.Select(s => s);

                        actualBDays = targetExactlyFull.ToList();


                        /// Я бы не стал удалять то, что закомментирвоано ниже. Не применяется в этом конкретном случае,
                        /// но может быть использовано за основу при других задачах
                        ...
                        // JToken personCard;
                        //   bool personCardExist = JObject.Parse(item.Value<string>()).TryGetValue("PersonCard", out personCard);
                        //    if (personCardExist)
                        //    {
                        //   var children = personCard.Children().Children();
                ...
                var json = JsonConvert.SerializeObject(actualBDays);
