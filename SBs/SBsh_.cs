...
namespace SBsh_
{
    ...
    static class ExplicitInterfacesCheck
    {
        public static void Check()
        {
            ...
            PrintIntDouble pid = (IPrintDouble)new PrintIntDouble();
            pid.Print(0.335);
            pid = (IPrintInt)new PrintIntDouble();
            pid.Print(0.123);
        }
    }
    ...
    public class LINQ_play
    {
        ...
        public LINQ_play()
        {
            ...
            var c = (from s in cups select new { s.RacerName, s.Position })
...
System.Diagnostics.Trace.WriteLine(e.Message);
    ...
    public class MongoDbCheck
    {
        public static void GO()
        {
            ...
            while (dbs.MoveNext())
            {
                foreach(BsonDocument str in dbs.Current)
                {
                    BsonElement b;
                    str.TryGetElement("name", out b);
                    if (b != null) {
                        dbNames.Add(b.Value.ToString());
                    }
                }
            }

            IMongoDatabase db = client.GetDatabase(dbNames[3]);
            string cmdDoc = (@"{ ""dbStats"": 1}");
            var cmd = new JsonCommand<BsonDocument>(cmdDoc);
            var res = db.RunCommand(cmd);
            res = db.RunCommand(new JsonCommand<BsonDocument>((@"{""show dbs""}")));
...
namespace SBsh_
{
    ...
    public class Helper
    {
        ...
        public void PrintList<T>(ICollection<T> list_)
        {
